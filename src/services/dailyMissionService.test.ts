...
class FakeQueryBuilder {
  ...
  in(_column: string, _values: Array<string | number>) {
    return this;
  }
  is(_column: string, _value: string | null) {
    return this;
  }
  ...
  maybeSingle() {
    return Promise.resolve({
      data: this.state[this.table] ?? null,
      error: null,
    });
  }
  single() {
...
class FakeSupabaseClient {
  public readonly rpcCalls: Array<{
    fn: string;
  ...
  async rpc(fn: string, args: Record<string, unknown>) {
    ...
    if (fn === "complete_all_dailies") {
      return {
        data: { already_completed: false, streak: 2, total: 5 },
        error: null,
      };
    }
...
describe("DailyMissionService", () => {
  it("assigns missions for the current day and auto-tracks a checkin", async () => {
    ...
    expect(
      admin.rpcCalls.some((call) => call.fn === "record_mission_progress"),
    ).toBe(true);
  });

  it("rejects progress updates for missions that are not assigned today", async () => {
    const admin = new FakeSupabaseClient({
      developers: { id: 8, claimed: true },
    });
    const service = new DailyMissionService(admin as never);
  ...
  it("claims rewards only when all missions are complete and prevents duplicates", async () => {
    ...
    const admin = new FakeSupabaseClient({
      ...
      daily_mission_progress: missions.map((mission) => ({
