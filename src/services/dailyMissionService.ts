...
export class DailyMissionService {
  ...
  async loadMissionSummary(
    ...
    const progressMap = new Map(
      (progressRows ?? []).map((r) => [String(r.mission_id), r]),
    );
  ...
  async updateProgress(payload: DailyMissionProgressPayload): Promise<unknown> {
    ...
    const increment =
      typeof payload.increment === "number" && payload.increment > 0
        ? payload.increment
        : 1;
    ...
    const mission = this.resolveMission(
      payload.developerId,
      payload.missionId,
  ...
  async claimReward(
    ...
    const { data: result, error: rpcError } = await this.admin.rpc(
    ...
    if (claimResult.total !== undefined && claimResult.total % 7 === 0) {
      ...
      if (!freezeError) {
        ...
        console.error(
          "[dailies] grant_streak_freeze error:",
          freezeError.message,
        );
      }
    }
  ...
  private resolveMission(
    ...
    return (
      getDailyMissions(developerId, today, false).find(
        (m) => m.id === missionId,
      ) ??
      getDailyMissions(developerId, today, isMobile).find(
        (m) => m.id === missionId,
      ) ??
      null
    );
