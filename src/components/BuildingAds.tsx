...
function AdLedWrap({
  ...
  useFrame(({ clock }) => {
    if (needsScroll)
      tex.offset.x = (clock.elapsedTime * SCROLL_SPEED * 0.8) % 1;
  });
  ...
  return (
    <group ref={groupRef}>
      {billboardAds.map((ad, i) => {
        ...
        const isDimmed =
          !!focusedLower &&
          loginLower !== focusedLower &&
          loginLower !== focusedBLower;
        return (
