...
const RaidVehiclePreview = dynamic(() => import("./RaidVehiclePreview"), {
  ssr: false,
});
...
function getScarcityInfo(item: ShopItem, soldCount: number) {
  ...
  if (item.available_until) {
    ...
    if (deadline <= now)
      return { expired: true, label: "Ended", color: "#666" };
  ...
  if (item.max_quantity != null) {
    ...
    if (remaining === 0)
      return { expired: true, label: "Sold out", color: "#666" };
    return {
      expired: false,
...
function savePendingBillboard(file: File): void {
  ...
  reader.onloadend = () => {
    try {
      ...
      console.warn("[ShopClient.tsx] failed to save pending billboard:", err);
    }
  };
  reader.readAsDataURL(file);
}

function getPendingBillboard(): {
  data: string;
  type: string;
  name: string;
} | null {
  try {
    ...
    console.warn("[ShopClient.tsx] failed to read pending billboard:", err);
    return null;
  }
}
...
function clearPendingBillboard(): void {
  try {
    ...
    console.warn("[ShopClient.tsx] failed to clear pending billboard:", err);
  }
}
...
function PixModal({
  ...
  useEffect(() => {
    ...
    pollRef.current = setInterval(async () => {
      try {
        ...
        console.warn(
          "[ShopClient.tsx] failed to poll PIX payment status:",
          err,
        );
      }
    }, 3000);
  ...
  const copyCode = useCallback(async () => {
    try {
      ...
      console.warn("[ShopClient.tsx] failed to copy PIX code:", err);
    }
  }, [data.brCode]);
      ...
      <div className="relative mx-4 w-full max-w-sm border-[2px] border-border bg-bg p-6">
        ...
        {status === "completed" ? (
          ...
          <>
            ...
            {/* PIX code + copy */}
            <div className="mb-4">
              <p className="mb-1 text-[8px] text-muted">
                PIX code (copy &amp; paste):
              </p>
  ...
  return (
    <div className="mt-2 flex items-center gap-3 border-[2px] border-border/50 bg-bg/50 px-3 py-2">
      ...
      {isOwned ? (
        ...
        <span className="ml-auto text-[9px] text-dim normal-case">
          Preview only
        </span>
      )}
    </div>
  ...
  const handleUpload = useCallback(
    async (slotIndex: number) => {
      ...
      try {
        ...
        console.error(
          "[ShopClient.tsx] failed to upload billboard image:",
          err,
        );
        // ignore
      } finally {
      ...
      {isOwned ? (
        <>
          ...
          <p className="mb-2 text-[9px] text-muted normal-case">
            {slotCount} billboard slot{slotCount !== 1 ? "s" : ""} — upload an
            image for each. Buy more to unlock more slots.
          </p>
        ...
        <>
          <p className="mb-2 text-[9px] text-muted normal-case">
            Try it — pick an image to preview on the 3D building. Purchase to
            save.
          </p>
...
export default function ShopClient({
  ...
  // Raid loadout state
  const [raidLoadout, setRaidLoadout] = useState<{
    vehicle: string;
    tag: string;
  ...
  const [buyingProvider, setBuyingProvider] = useState<
    "stripe" | "nowpayments" | "abacatepay" | "cashfree" | "points" | null
  >(null);
  ...
  const [starVerifyStep, setStarVerifyStep] = useState<
    "idle" | "opened" | "verifying"
  >("idle");
  const [activeTab, setActiveTab] = useState<
    "building" | "raid" | "consumables" | "points" | "relics"
  >(() => {
    if (
      purchasedItem &&
      [...RAID_VEHICLE_ITEMS, ...RAID_TAG_ITEMS, ...RAID_BOOST_ITEMS].includes(
        purchasedItem,
      )
    )
      return "raid";
    if (purchasedItem && RAID_CONSUMABLE_ITEMS.includes(purchasedItem))
      return "consumables";
    return "building";
  });
  ...
  useEffect(() => {
    if (activeTab === "relics") {
      ...
      fetch("/api/relics")
        ...
        .catch((err) => {
          console.error("[ShopClient.tsx] failed to load relics:", err);
  ...
  const handleEquipRelic = async (relicId: string | null) => {
    ...
    try {
      ...
      console.error("[ShopClient.tsx] failed to equip relic:", err);
    } finally {
  ...
  const [pendingCashfreeItem, setPendingCashfreeItem] = useState<string | null>(
    null,
  );
  const [customColor, setCustomColor] = useState<string | null>(
    initialCustomColor,
  );
  const [ledBannerText, setLedBannerText] = useState<string | null>(
    initialLedBannerText ?? null,
  );
  const [selectedTitle, setSelectedTitle] = useState<string | null>(
    initialSelectedTitle ?? "auto",
  );
  ...
  const [previewLedBannerText, setPreviewLedBannerText] = useState<
    string | null
  >(null);
  const [previewBillboardImages, setPreviewBillboardImages] = useState<
    string[] | null
  >(null);
  const [savingCustomization, setSavingCustomization] = useState<string | null>(
    null,
  );
  const [savedCustomization, setSavedCustomization] = useState<string | null>(
    null,
  );
  const [autoUploading, setAutoUploading] = useState(false);
  const [purchaseToast, setPurchaseToast] = useState<string | null>(
    purchasedItem,
  );
  const [giftToast, setGiftToast] = useState<{
    item: string;
    to: string;
  } | null>(giftedItem && giftedTo ? { item: giftedItem, to: giftedTo } : null);
  ...
  const effectiveLoadout: Loadout = {
    crown:
      loadout.crown ??
      (!initialLoadout && owned.includes("flag") ? "flag" : null),
    roof: loadout.roof,
    aura: loadout.aura,
    faces: loadout.faces,
  };
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("[ShopClient.tsx] failed to save billboard override:", err);
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        try {
          ...
          console.warn(
            "[ShopClient.tsx] failed to cache loadout in localStorage:",
            e,
          );
        }
      }
  ...
  const handleSaveLoadout = useCallback(async () => {
    ...
    try {
      ...
      console.error("[ShopClient.tsx] failed to save loadout:", err);
      setError("Failed to save. Try again.");
    } finally {
      setSaving(false);
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        try {
          ...
          console.warn(
            "[ShopClient.tsx] failed to cache raid loadout in localStorage:",
            err,
          );
        }
      }
  ...
  const claimFreeItem = useCallback(async () => {
    ...
    try {
      ...
      console.error("[ShopClient.tsx] failed to claim free item:", err);
  ...
  const verifyLeetCodeStar = useCallback(async () => {
    ...
    try {
      ...
      if (data.verified) {
        setOwned((prev) =>
          prev.includes("github_star") ? prev : [...prev, "github_star"],
        );
      ...
      console.error("[ShopClient.tsx] failed to verify repository star:", err);
      setError("Network error. Try again.");
      setStarVerifyStep("opened");
    } finally {
  ...
  const checkout = useCallback(
    async (
      itemId: string,
      provider: "stripe" | "nowpayments" | "abacatepay" | "cashfree" = "stripe",
      phoneVal?: string,
    ) => {
      if (buyingItem) return;
      ...
      try {
        ...
        if (data.paymentSessionId) {
          ...
          try {
            ...
            console.error(
              "[ShopClient.tsx] failed to initialize Cashfree SDK:",
              sdkErr,
            );
        ...
        console.error(
          "[ShopClient.tsx] failed to start Cashfree checkout:",
          err,
        );
        setError("Network error. Try again.");
      } finally {
  ...
  const handleBuyWithPoints = useCallback(
    async (itemId: string) => {
      ...
      try {
        ...
        console.error("[ShopClient.tsx] failed to start PIX checkout:", err);
        setError("Network error. Try again.");
      } finally {
        setBuyingItem(null);
        setBuyingProvider(null);
    ...
    try {
      ...
      if (res.ok) {
        if (!isXPCode && !isAllItemsCode && data.item_id) {
          setOwned((prev) =>
            prev.includes(data.item_id) ? prev : [...prev, data.item_id],
          );
        }
        if (isAllItemsCode && Array.isArray(data.granted_items)) {
          // Add all newly unlocked items to local owned state
          setOwned((prev) => {
            const newItems = (data.granted_items as string[]).filter(
              (id) => !prev.includes(id),
            );
        ...
        if (isXPCode && (data.xp_granted || data.new_xp_total)) {
          if (data.new_xp_total) {
            ...
            setXpLevel((prev) =>
              Math.max(
                prev,
                levelFromXp((prev === 1 ? 0 : prev * 25) + data.xp_granted),
              ),
            );
          }
          fetch("/api/me")
      ...
      console.error("[ShopClient.tsx] failed to redeem code:", err);
      setRedeemState("error");
        ...
        if (itemId) {
          ...
          if (itemId === "billboard") {
            ...
            if (pending) {
              ...
              formData.append("file", file);
              formData.append("slot_index", "0");
              fetch("/api/customizations/upload", {
                method: "POST",
                body: formData,
              })
                .then(async (res) => {
      ...
      {giftToast && (
        <div className="fixed top-4 left-1/2 z-50 -translate-x-1/2">
          ...
          >
            ...
            <span>
              {ITEM_NAMES[giftToast.item] ?? giftToast.item} sent to{" "}
              {giftToast.to}!
            </span>
          </div>
        </div>
      ...
      {buyingItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="border-[3px] border-border bg-bg p-6 text-center">
            <div className="mb-3 text-2xl animate-pulse">
              {ITEM_EMOJIS[buyingItem] ?? "🛒"}
            </div>
            <p className="text-xs text-cream">
              {buyingProvider === "abacatepay"
                ? "Generating PIX..."
                : buyingProvider === "cashfree"
                  ? "Opening UPI..."
      ...
      <div className="mb-5 border-[2px] border-border bg-bg-raised p-4">
        ...
        <div className="flex gap-2">
          <input
            ...
            onKeyDown={(e) =>
              e.key === "Enter" && redeemState === "idle" && handleRedeem()
            }
            maxLength={30}
          ...
          <button
            ...
            style={{
              backgroundColor: redeemState === "success" ? "#39d353" : ACCENT,
              boxShadow: `2px 2px 0 0 ${SHADOW}`,
            }}
          >
            {redeemState === "loading"
        ...
        {redeemMsg && (
          <p
            className={`mt-2 text-[9px] normal-case ${redeemState === "success" ? "text-green-400" : "text-red-400"}`}
          >
            {redeemMsg}
          </p>
        )}
                    ...
                    <div className="border-[3px] border-border bg-bg-raised p-4">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className={`flex-1 border-[2px] py-2 text-[10px] transition-all ${
                            bStyle === "tower"
                      ...
                      <p className="mt-2 text-[8px] text-dim normal-case">
                        *Exclusive dev option: switch between high-rise tower
                        and sprawling bungalow layout.
                      </p>
                    </div>
                  )}
                  ...
                  {(Object.entries(ZONE_ITEMS) as [string, string[]][]).map(
                    ([zone, zoneItemIds]) => {
                      ...
                      const equippedName = equippedId
                        ? (ITEM_NAMES[equippedId] ?? equippedId)
                        : "None";
                      const ownedCount = zoneItemIds.filter((id) =>
                        owned.includes(id),
                      ).length;
                      ...
                      return (
                        <div
                          key={zone}
                          className="border-[3px] border-border bg-bg-raised p-4"
                        >
                          {/* Zone header */}
                          <div className="flex items-center justify-between mb-3">
                            <h3 className="text-sm" style={{ color: ACCENT }}>
                            ...
                            <span className="text-[9px] text-muted normal-case">
                              {ownedCount}/{zoneItemIds.length} owned ·
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                            {zoneItemIds.map((itemId) => {
                              ...
                              const hasAchievement =
                                achUnlock &&
                                achievements.includes(achUnlock.achievement);
                              ...
                              if (isEquipped) {
                                ...
                                itemId === "scouting_satellite" &&
                                !isOwned
                              ) {
                                const met =
                                  acceptedMedium >= 10 || acceptedHard >= 5;
                                ...
                                achUnlock &&
                                !shopItem?.price_usd_cents
                              ) {
                                badge = hasAchievement
                                  ? "Unlockable!"
                                  : achUnlock.label.split("(")[0].trim();
                                badgeColor = hasAchievement
                                  ? "#39d353"
                                  : "#a0a0b0";
                              } else if (shopItem) {
                              ...
                              const handleClick = () => {
                                ...
                                if (isEquipped) {
                                  ...
                                  if (starVerifyStep === "idle") {
                                    window.open(
                                      "https://github.com/Ixotic27/The-Leetcode-City",
                                      "_blank",
                                    );
                                    setStarVerifyStep("opened");
                              ...
                              return (
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    {(purchaseCounts[itemId] ?? 0) >= 3 &&
                                      !isOwned && (
                                        <span className="mt-0.5 text-[8px] text-dim">
                                          {purchaseCounts[itemId]} purchased
                                          this week
                                        </span>
                                      )}
                                  </button>
                                  ...
                                  {isConfirming && shopItem && (
                                    ...
                                    >
                                      ...
                                      <div className="flex flex-col gap-1">
                                        ...
                                        {shopItem &&
                                          shopItem.price_points != null && (
                                            <button
                                              onClick={(e) => {
                                                e.stopPropagation();
                                                setConfirmBuyItem(null);
                                                handleBuyWithPoints(itemId);
                                              }}
                                              disabled={
                                                isBuying ||
                                                points <
                                                  (shopItem.price_points ?? 0)
                                              }
                                            ...
                                            >
                                              {isBuying
                                                ...
                                                : points <
                                                    (shopItem.price_points ?? 0)
                                                  ? `Not enough points (${shopItem.price_points})`
                                                  : `Buy with ${shopItem.price_points} Points`}
                                            </button>
                                          )}
                                      </div>
                                    </div>
                          ...
                          {equippedId === "custom_color" &&
                            (owned.includes("custom_color") ||
                              (isDevAccount && devModeEnabled)) && (
                              <div className="mt-3 border-[2px] border-border/50 bg-bg/50 px-4 py-3">
                                <p className="mb-2 text-[9px] text-muted normal-case">
                                  Custom Building Color
                                </p>
                                <div className="flex items-center gap-3">
                                  <input
                                    type="color"
                                  ...
                                  />
                                  <span className="flex-1 text-[10px] text-cream font-mono">
                                    {customColor ?? "#ffffff"}
                                  </span>
                                  <button
                                    onClick={() =>
                                      handleSaveCustomization("custom_color", {
                                        color: customColor,
                                      })
                                    }
                                    disabled={
                                      savingCustomization === "custom_color"
                                    }
                                    className="btn-press px-4 py-1.5 text-[9px] text-bg disabled:opacity-40"
                                    style={{
                                      backgroundColor:
                                        savedCustomization === "custom_color"
                                          ? "#39d353"
                                          : ACCENT,
                                      boxShadow: `1px 1px 0 0 ${SHADOW}`,
                                    }}
                          ...
                          {equippedId === "led_banner" &&
                            (owned.includes("led_banner") ||
                              (isDevAccount && devModeEnabled)) && (
                              <div className="mt-3 border-[2px] border-border/50 bg-bg/50 px-4 py-3">
                                <p className="mb-2 text-[9px] text-muted normal-case">
                                  LED Banner Text
                                ...
                                <div className="flex items-center gap-3">
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setLedBannerText(e.target.value)
                                    }
                                  ...
                                  <button
                                    onClick={() =>
                                      handleSaveCustomization("led_banner", {
                                        text: ledBannerText,
                                      })
                                    }
                                    disabled={
                                      savingCustomization === "led_banner"
                                    }
                                    className="btn-press px-4 py-1.5 text-[9px] text-bg disabled:opacity-40"
                                    style={{
                                      backgroundColor:
                                        savedCustomization === "led_banner"
                                          ? "#39d353"
                                          : ACCENT,
                                      boxShadow: `1px 1px 0 0 ${SHADOW}`,
                                    }}
                          ...
                          {equippedId === "billboard" &&
                            (owned.includes("billboard") ||
                              (isDevAccount && devModeEnabled)) && (
                              <div className="mt-3">
                          ...
                          <div className="relative" data-buy-popover>
                            <button
                              onClick={() => {
                                ...
                                setConfirmBuyItem(
                                  isConfirming ? null : "streak_freeze",
                                );
                              }}
                                  ...
                                  <div className="flex gap-1">
                                    ...
                                    <button
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        setConfirmBuyItem(null);
                                        checkout("streak_freeze", "cashfree");
                                      }}
                                      disabled={isBuying}
                                      className="btn-press flex-1 py-1 text-[9px] text-bg disabled:opacity-40"
                                      style={{
              ...
              <div className="space-y-6">
                <div className="border-[3px] border-border bg-bg-raised p-4">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-[9px] text-muted normal-case">
                        Redeem points for exclusive items and power-ups.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                    {items
                      .filter(
                        (item) =>
                          item.price_points != null && item.price_points > 0,
                      )
                      .map((item) => {
                          ...
                          >
                            <button
                              onClick={() => {
                                ...
                                const zone = (
                                  Object.keys(ZONE_ITEMS) as (keyof Loadout)[]
                                ).find((z) => ZONE_ITEMS[z].includes(itemId));
                                if (zone) {
                              ...
                              className={[
                                ...
                                isOwned && !isConsumable
                                  ? "border-[#39d353] bg-[rgba(57,211,83,0.1)]"
                                  : "border-border bg-bg-card",
                                isConfirming
                                  ? "border-cream"
                                  : "hover:border-border-light",
                                (isOwned && !isConsumable) || isMaxed
                                  ? "opacity-60"
                                  : "",
                              ].join(" ")}
                            >
                            ...
                            {isConfirming && (
                              <div className="absolute left-1/2 -translate-x-1/2 top-full mt-1 z-30 w-36 border-[2px] border-border bg-bg p-2 shadow-lg text-center">
                                <p className="text-[9px] text-cream mb-1.5">
                                  Redeem {ITEM_NAMES[itemId]}?
                                </p>
                                <p
                                ...
                                {!(
                                  ...
                                  <p className="mt-1.5 text-[8px] text-red-400 normal-case">
                                    Not enough points!
                                  </p>
                                )}
                              </div>
                ...
                <div className="p-4 border-[2px] border-dashed border-border/40 bg-bg-card/50 text-center">
                  <p className="text-[10px] text-muted normal-case italic">
                    Earn points via daily check-ins (+5pts) and daily tasks
                    (+15pts).
                  </p>
                </div>
              </div>
                      ...
                      <div key={itemId} className="relative">
                        ...
                        >
                          <div
                            ...
                            style={{
                              backgroundColor: TAG_COLORS[itemId] ?? "#fff",
                            }}
                          />
          ...
          <div className="border-[3px] border-border bg-bg-raised p-4">
            ...
            <div className="grid grid-cols-2 gap-3 mb-0">
              {RAID_CONSUMABLE_ITEMS.map((itemId) => {
                ...
                const isAccessible =
                  !isLevelLocked || (isDevAccount && devModeEnabled);

                // Get inventory counts
                const inventory = consumablesInventory.find(
                  (c) => c.item_id === itemId,
                );
                ...
                if (inventory?.last_reset_week) {
                  ...
                  const currentWeekStr = new Date(
                    ...
                    .split("T")[0];
                  if (
                    new Date(inventory.last_reset_week)
                      .toISOString()
                      .split("T")[0] !== currentWeekStr
                  ) {
                    weeklyUses = 0;
                  }
                }
                  ...
                  <div key={itemId} className="relative">
                    <div
                      className={[
                        "w-full overflow-hidden transition-colors border-[2px]",
                        isAccessible
                          ? "border-[#ffaa00]/40 bg-[rgba(255,170,0,0.05)]"
                          : "border-border opacity-70 bg-bg-card",
            ...
            <div className="mt-4 p-4 border-[2px] border-dashed border-[#ffaa00]/30 bg-[#ffaa00]/5 text-center">
              <p className="text-[10px] text-muted normal-case italic">
                Each offensive or defensive item has a strict global limit: 3
                uses per player, per week. Defenses automatically equip while
                you&apos;re offline to block incoming Raids unless EMP&apos;d.
                Use sabotage viruses and EMP devices wisely before executing a
                raid!
              </p>
            </div>
          </div>
            ...
            {loadingRelics ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
                ...
                <div className="md:col-span-3 space-y-3">
                  <p className="text-[8px] uppercase tracking-wider text-muted font-bold">
                    Resonators
                  </p>
                        ...
                        >
                          ...
                          >
                            ...
                            <span
                              className="text-base"
                              style={{
                                color: isLocked ? "#71717a" : eraTheme.border,
                              }}
                            >
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <h4 className="text-[10px] font-bold text-cream mt-0.5">
                              {selectedRelic.name}
                            </h4>
                          </div>
                        </div>
                        ...
                        <div className="space-y-1">
                          <span className="text-[8px] text-muted font-bold tracking-wider">
                            ABILITIES
                          </span>
