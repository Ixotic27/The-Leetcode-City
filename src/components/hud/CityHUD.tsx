    ...
    <>
                ...
                <SearchFeedback
                  ...
                  onRetry={() => {
                    window.dispatchEvent(
                      new CustomEvent("leetcodecity:search"),
                    );
                  }}
                />
              </div>
              ...
              {buildings.length > 0 && (
                <div className="pointer-events-auto flex flex-col items-center gap-3 mt-3 w-full max-w-lg">
                  ...
                  <div className="flex w-full items-center gap-3 sm:gap-4">
                    ...
                    >
                      Explore City
                      <span className="block text-[8px] opacity-60 normal-case font-normal">
                        Browse Buildings
                      </span>
                    </button>
                    <button
          ...
          <div className="pointer-events-auto flex flex-col items-center gap-2">
            ...
            {!isMobile && buildings.length > 0 && (
              <p className="hidden sm:block text-[8px] text-cream/25 tracking-wider">
                Press{" "}
                <kbd className="px-1 py-0.5 border border-border/40 rounded text-[7px] text-cream/40 mx-0.5">
                  E
                </kbd>{" "}
                to explore ·{" "}
                <kbd className="px-1 py-0.5 border border-border/40 rounded text-[7px] text-cream/40 mx-0.5">
        ...
        <div className="fixed top-20 left-1/2 z-50 -translate-x-1/2 w-[calc(100%-1.5rem)] max-w-xs animate-[slide-up_0.2s_ease-out]">
          <div className="border-[3px] border-border bg-bg-raised/95 px-4 py-3 backdrop-blur-sm">
            ...
            <button
              onClick={async () => {
                try {
                  const { trackSignInPromptClicked } =
                    await import("@/lib/himetrica");
      ...
      {!flyMode && !introMode && !rabbitCinematic && feedEvents.length >= 1 && (
        <ActivityTicker
          ...
          renewalProgress={
            stats?.renewal_raised_inr !== undefined &&
            stats?.renewal_target_inr !== undefined
              ? {
                  raised: stats.renewal_raised_inr,
                  target: stats.renewal_target_inr,
                }
              : null
          }
          onEventClick={(evt) => {
