...
const CATEGORIES: CategoryDef[] = [
  ...
  },
  {
    ...
    label: "Top",
    slot: "top",
    loadoutKey: "clothes_top_id",
    colorKey: "clothes_top_color",
  },
  {
    id: "bottom",
    label: "Bottom",
    slot: "bottom",
    loadoutKey: "clothes_bottom_id",
    colorKey: "clothes_bottom_color",
  },
  {
    id: "shoes",
    label: "Shoes",
    slot: "shoes",
    loadoutKey: "shoes_id",
    colorKey: "shoes_color",
  },
  {
    id: "costume",
    label: "Costumes",
    slot: "costume",
    loadoutKey: "clothes_full_id",
    colorKey: "clothes_full_color",
    optional: true,
  },
  {
    id: "hat",
    label: "Hat",
    slot: "hat",
    loadoutKey: "acc_hat_id",
    colorKey: "acc_hat_color",
    optional: true,
  },
  {
    id: "glasses",
    label: "Glasses",
    slot: ["face", "mask"],
    loadoutKey: "acc_face_id",
    colorKey: "acc_face_color",
    optional: true,
  },
  {
    id: "facial",
    label: "Facial",
    slot: "facial",
    loadoutKey: "acc_facial_id",
    colorKey: "acc_facial_color",
    optional: true,
  },
  {
    id: "jewelry",
    label: "Jewelry",
    slot: "jewelry",
    loadoutKey: "acc_jewelry_id",
    colorKey: "acc_jewelry_color",
    optional: true,
  },
  {
    id: "face",
    label: "Face",
    slot: ["blush", "lipstick", "eyes"],
    loadoutKey: "__face",
  },
  {
    id: "pets",
...
function PurchaseSummary({
  ...
  const visibleSlotKeys: (keyof AvatarLoadout)[] = [
    ...
    ...(hasCostume
      ...
      : [
          "clothes_top_id" as keyof AvatarLoadout,
          "clothes_bottom_id" as keyof AvatarLoadout,
          "shoes_id" as keyof AvatarLoadout,
        ]),
    "acc_hat_id",
    "acc_face_id",
    "acc_facial_id",
      ...
      {hasPurchases && (
        <div className="mb-2 space-y-0.5">
          ...
          <div className="flex justify-between items-center pt-1.5 border-t border-gray-200 mt-1">
            ...
            <span
              className={`text-xs font-bold ${canAfford ? "text-gray-800" : "text-red-500"}`}
            >
              {canAfford
                ? `${remaining.toLocaleString()} PX left`
            ...
            confirming ? (
              <>
                <div className="text-[10px] text-center text-amber-700 mb-1">
                  Buy {toBuy.length} item{toBuy.length > 1 ? "s" : ""} for{" "}
                  {totalCost} PX?
                </div>
...
export default function AvatarEditor({
  ...
  const drawPreview = useCallback(() => {
    ...
    if (loadout.blush_id) {
      ...
      if (info?.file)
        drawLayer(
          info.file,
          loadout.blush_color ?? info.default_color,
          info.no_tint,
        );
    }
    if (loadout.lipstick_id) {
      ...
      if (info?.file)
        drawLayer(
          info.file,
          loadout.lipstick_color ?? info.default_color,
          info.no_tint,
        );
    }
    ...
    if (loadout.clothes_full_id) {
      ...
      if (info?.file)
        drawLayer(
          info.file,
          loadout.clothes_full_color ?? info.default_color,
          info.no_tint,
        );
    } else {
      for (const [key, colorKey] of [
        ...
        if (info?.file)
          drawLayer(
            info.file,
            loadout[colorKey] ?? info.default_color,
            info.no_tint,
          );
      }
    }
    ...
    if (loadout.hair_id) {
      ...
      if (info?.file)
        drawLayer(
          info.file,
          loadout.hair_color ?? info.default_color,
          info.no_tint,
        );
    }
  ...
  const drawThumbnail = useCallback(
    (canvas: HTMLCanvasElement, item: ShopItem) => {
      if (!item.file) return;
      const ctx = canvas.getContext("2d");
      if (!ctx) return;
      ctx.imageSmoothingEnabled = false;
      ...
      if (item.slot === "pet") {
        ...
        if (img)
          ctx.drawImage(img, 0, 0, 18, 18, 0, 0, canvas.width, canvas.height);
      } else {
        drawFromAtlas(
          ctx,
          item.file,
  ...
  const setItemCanvasRef = useCallback(
    (el: HTMLCanvasElement | null, item: ShopItem) => {
      if (!el) return;
  ...
  const equipItem = (item: ShopItem) => {
    ...
    if (cat.optional && newLoadout[key] === item.id) {
      ...
      if (cat.colorKey)
        (newLoadout as Record<string, unknown>)[cat.colorKey] = null;
    } else {
      ...
      if (cat.colorKey) {
        (newLoadout as Record<string, unknown>)[cat.colorKey] =
          item.default_color ?? null;
      }
    }
  ...
  const equipFaceItem = (item: ShopItem) => {
    ...
    if (item.slot === "blush") {
      ...
      newLoadout.lipstick_id =
        newLoadout.lipstick_id === item.id ? null : item.id;
      newLoadout.lipstick_color = newLoadout.lipstick_id
        ? item.default_color
        : null;
    }
    setLoadout(newLoadout);
  ...
  const [userSelectedIds, setUserSelectedIds] = useState<Set<string>>(
    new Set(),
  );
            ...
            <div className="flex-1 overflow-y-auto p-3">
              {!imagesReady ? (
                ...
                <div className="grid grid-cols-3 gap-2">
                  {items
                    .filter((i) =>
                      ["blush", "lipstick", "eyes"].includes(i.slot),
                    )
                    .map((item) => {
                      const isActive =
                        item.slot === "blush"
                          ? loadout.blush_id === item.id
                          : item.slot === "lipstick"
                            ? loadout.lipstick_id === item.id
                            : true;
                      return (
                        <button
                          key={item.id}
                  ...
                  {PX_PACKAGES.map((pkg) => (
                    ...
                    >
                      <div>
                        ...
                        {pkg.badge && (
                          <span className="ml-2 text-[9px] bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded">
                            {pkg.badge}
                          </span>
                        )}
                      </div>
                  ...
                  {activeCat?.optional && (
                    <button
                      onClick={() => {
                        ...
                        const newLoadout = { ...loadout };
                        (newLoadout as Record<string, unknown>)[key] = null;
                        if (activeCat.colorKey)
                          (newLoadout as Record<string, unknown>)[
                            activeCat.colorKey
                          ] = null;
                        setLoadout(newLoadout);
                      }}
