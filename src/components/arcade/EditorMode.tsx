...
const CATALOG: CatalogItem[] = [
  ...
  ...["cream", "terra", "olive", "teal"].flatMap((c) => [
    ...
    seatItem(
      `sofa_2seat_${c}_front`,
      `2S ${c} Front`,
      "Sofas",
      `sofa_2seat_${c}_front.png`,
      32,
      32,
      2,
      2,
    ...
    seatItem(
      `sofa_2seat_${c}_right`,
      `2S ${c} Right`,
      "Sofas",
      `sofa_2seat_${c}_right.png`,
      16,
      32,
      1,
      2,
    ...
    seatItem(
      `sofa_3seat_${c}_right`,
      `3S ${c} Right`,
      "Sofas",
      `sofa_3seat_${c}_right.png`,
      16,
      48,
      1,
      3,
    ...
    seatItem(
      `sofa_1seat_${c}_right`,
      `1S ${c} Right`,
      "Sofas",
      `sofa_1seat_${c}_right.png`,
      16,
      16,
      1,
      1,
  ...
  ...Array.from({ length: 4 }, (_, i) =>
    item(
      `coffeetable_wood_${i + 1}`,
      `Wood Table ${i + 1}`,
      "Coffee",
      `coffeetable_wood_${i + 1}.png`,
      32,
      16,
      2,
      1,
  ...
  ...Array.from({ length: 3 }, (_, i) =>
    item(
      `coffeetable_glass_${i + 1}`,
      `Glass Table ${i + 1}`,
      "Coffee",
      `coffeetable_glass_${i + 1}.png`,
      32,
      16,
      2,
      1,
  ...
  ...[
    ...
    seatItem(
      `chair_wood_${c}_front`,
      `${c} (F)`,
      "Chairs",
      `chair_wood_${c}_front.png`,
      16,
      32,
      1,
      2,
  ...
  ...[
    ...
    seatItem(
      `chair_cushion_${c}_front`,
      `Cushion ${c}`,
      "Chairs",
      `chair_cushion_${c}_front.png`,
      16,
      32,
      1,
      2,
  ...
  ...[
    ...
    seatItem(
      `chair_office_${c}_front`,
      `Office ${c} (F)`,
      "Office",
      `chair_office_${c}_front.png`,
      16,
      32,
      1,
      2,
    ...
    seatItem(
      `chair_office_${c}_side`,
      `Office ${c} (S)`,
      "Office",
      `chair_office_${c}_side.png`,
      16,
      32,
      1,
      2,
  ...
  ...[
    ...
    "pink",
    "lavender",
  ].map((c) => item(`tv_${c}`, `TV ${c}`, "TVs", `tv_${c}.png`, 16, 32, 1, 2)),
  ...
  // ─── Bookshelves ────────────────────────────────────────────
  ...["birch", "oak", "walnut", "cherry", "mahogany", "maple", "dark"].map(
    (c) =>
      item(
        `bookshelf_${c}`,
        `Shelf ${c}`,
...
export default function EditorMode({
  ...
  const getFurnitureOccupied = useCallback(
    (furn: PlacedFurniture[], exclude?: string): Set<string> => {
      ...
      if (itm.placement === "wall") {
        ...
        for (let dy = 0; dy < rh; dy++) {
          for (let dx = 0; dx < rw; dx++) {
            if (getTileType(map, tileX + dx, tileY + dy) !== "wall")
              return false;
          }
        }
        return true;
  ...
  const getGhostImg = useCallback((file: string): HTMLImageElement | null => {
    if (ghostImgCache.current.has(file))
      return ghostImgCache.current.get(file)!;
    const img = new Image();
  ...
  useEffect(() => {
    ...
    overlayCanvas.style.cssText =
      "position:absolute;top:0;left:0;width:100%;height:100%;pointer-events:none;image-rendering:pixelated;z-index:55;";
    ...
    const drawOverlay = () => {
      ...
      if (showDebugRef.current) {
        ...
        for (const f of furnitureRef.current) {
          ...
          ctx.fillText(
            f.sprite.replace(/sofa_|chair_|plant_/g, "").slice(0, 8),
            f.x + f.width / 2,
            f.y + 8,
          );
        }
      ...
      if (ed.selectedItem && ed.ghostTileX >= 0) {
        ...
        const validColor = ed.ghostValid
          ? "rgba(80, 200, 80,"
          : "rgba(200, 80, 80,";
        if (ed.selectedItem.placement === "wall") {
          ctx.fillStyle = ed.ghostValid
            ? "rgba(80, 140, 240, 0.2)"
            : "rgba(200, 80, 80, 0.25)";
      ...
      if (ed.selectedFurniture) {
        const f = furnitureRef.current.find(
          (f) => f.id === ed.selectedFurniture,
        );
        if (f) {
          ...
          const isSittable1x1 =
            (f.sittable ||
              ...
              f.sprite.includes("1seat_")) &&
            f.width <= ts &&
            f.height <= ts;
          const dirLabel = isSittable1x1
            ? ` · R=face:${(f as PlacedFurniture).sitDir ?? "down"}`
            : "";
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      if (ed.selectedItem) {
        setEditor((prev) => ({
          ...
          ghostTileX: tx,
          ghostTileY: ty,
          ghostValid: canPlace(ed.selectedItem!, tx, ty),
        }));
      }
      ...
      if (ed.selectedFurniture && !ed.selectedItem) {
        ...
        if (f) {
          const catItem = CATALOG.find(
            (c) => c.id === f.catalogId || c.id === f.sprite,
          );
          if (catItem) {
    ...
    const onClick = (e: MouseEvent) => {
      ...
      if (ed.selectedFurniture) {
        ...
        if (f) {
          ...
          const onSelf =
            tx >= f.x / ts &&
            tx < f.x / ts + f.width / ts &&
            ty >= f.y / ts &&
            ty < f.y / ts + f.height / ts;
                ...
                prev.map((item) =>
                  item.id === f.id
                    ? {
                        ...
                        y: ty * ts,
                        width: catItem.tw * ts,
                        height: catItem.th * ts,
                        sortY: ty * ts + catItem.th * ts,
                      }
                    : item,
                ),
              );
      ...
      const clicked = furnitureRef.current.find(
        (f) =>
          tx >= f.x / ts &&
          tx < f.x / ts + f.width / ts &&
          ty >= f.y / ts &&
      ...
      setEditor((prev) => ({
        ...prev,
        selectedFurniture: clicked?.id ?? null,
        selectedItem: null,
        ghostTileX: -1,
      }));
    };
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (
        ...
        ed.selectedFurniture
      ) {
        e.preventDefault();
        setUndoStack((prev) => [...prev, furnitureRef.current]);
        setFurniture((prev) =>
          prev.filter((f) => f.id !== ed.selectedFurniture),
        );
        // Also remove linked interactive object
        map.objects = map.objects.filter(
          (o) =>
            (o as unknown as Record<string, unknown>)._furnitureId !==
            ed.selectedFurniture,
        );
  ...
  return (
    <div className="absolute inset-0 z-[60] pointer-events-none">
      ...
      >
        <span className="text-[11px] text-yellow-400 font-bold tracking-wide uppercase">
          Edit Mode
        </span>
        <span className="text-white/20">|</span>
        <button
          onClick={() => setShowGrid((g) => !g)}
          className={`cursor-pointer text-[10px] px-2 py-0.5 rounded transition-colors ${showGrid ? "text-white bg-white/15" : "text-white/50 hover:text-white/80"}`}
        >
          Grid
        </button>
        <button
          onClick={() => setShowDebug((d) => !d)}
          className={`cursor-pointer text-[10px] px-2 py-0.5 rounded transition-colors ${showDebug ? "text-orange-400 bg-orange-400/15" : "text-white/50 hover:text-white/80"}`}
        >
          Debug
        </button>
        <button
          ...
          className="cursor-pointer text-[10px] text-white/50 hover:text-white/80 px-2 py-0.5 rounded disabled:opacity-30 transition-colors"
        >
          Undo
        </button>
        <button
          onClick={handleSave}
          disabled={saving}
          className="cursor-pointer text-[10px] text-green-400 hover:text-green-300 font-medium px-2 py-0.5 rounded bg-green-400/10 hover:bg-green-400/20 transition-colors disabled:opacity-50"
        >
          {saving ? "Saving..." : "Save"}
        </button>
        <button
          onClick={onExit}
          className="cursor-pointer text-[10px] text-white/50 hover:text-white/80 px-2 py-0.5 rounded transition-colors"
        >
          Exit
        </button>
      </div>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-1.5">
            ...
            {filteredItems.map((itm) => (
              ...
              >
                ...
                >
                  <SpritePreview
                    file={itm.file}
                    animated={itm.animated}
                    pw={itm.pw}
                    ph={itm.ph}
                  />
                </div>
                <span className="text-[9px] text-white/60 leading-tight text-center">
                  {itm.label}
                </span>
                {itm.placement !== "floor" && (
                  <span
                    className={`text-[7px] px-1 rounded-full ${itm.placement === "wall" ? "bg-blue-500/20 text-blue-400" : "bg-purple-500/20 text-purple-400"}`}
                  >
                    {itm.placement}
                  </span>
                )}
          ...
          {filteredItems.length === 0 && (
            <p className="text-[11px] text-white/25 text-center py-8">
              No items found
            </p>
          )}
        </div>
          ...
          <div className="p-2 border-t border-white/10">
            ...
            <button
              onClick={() => {
                setUndoStack((prev) => [...prev, furnitureRef.current]);
                setFurniture((prev) =>
                  prev.filter((f) => f.id !== editor.selectedFurniture),
                );
                map.objects = map.objects.filter(
                  (o) =>
                    (o as unknown as Record<string, unknown>)._furnitureId !==
