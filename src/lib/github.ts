...
function isV2Dev(dev: DeveloperRecord): boolean {
  ...
  const isLcPlaceholder =
    dev.contributions_total === 1000 &&
    dev.active_days_last_year === 365 &&
    !dev.account_created_at;
...
export function generateCityLayout(devs: DeveloperRecord[]): {
  ...
  function gridToWorld(gx: number, gz: number): [number, number] {
    ...
    if (gz >= 0) {
      ...
      wz =
        -(positiveIdx * BLOCK_FOOTPRINT_Z + positiveIdx * STREET_W) -
        RIVER_PUSH;
    }
    return [wx, wz];
  }
  ...
  function placeBlockContent(
    ...
    for (let i = 0; i < blockDevs.length; i++) {
      ...
      buildings.push({
        ...
        led_banner_text:
          ((dev as unknown as Record<string, unknown>).led_banner_text as
            string | null) ?? null,
        achievements:
        ...
        easy_solved:
          ((dev as unknown as Record<string, unknown>).easy_solved as number) ??
          undefined,
        medium_solved:
          ((dev as unknown as Record<string, unknown>)
            .medium_solved as number) ?? undefined,
        hard_solved:
          ((dev as unknown as Record<string, unknown>).hard_solved as number) ??
          undefined,
        acceptance_rate:
          ((dev as unknown as Record<string, unknown>)
            .acceptance_rate as number) ?? undefined,
        contest_rating:
          ((dev as unknown as Record<string, unknown>)
            .contest_rating as number) ?? undefined,
        lc_streak:
          ((dev as unknown as Record<string, unknown>).lc_streak as number) ??
          undefined,
      });
    }
  ...
  function placeSpiralCluster(
    ...
    function getQuadrantBlockWorld(
      gx: number,
      gz: number,
      signX: number,
      signZ: number,
  ...
  for (const b of allBlocks)
    blockByGrid.set(`${b.district}:${b.signX}:${b.signZ}:${b.gx},${b.gz}`, b);
  for (const block of allBlocks) {
    const halfX = BLOCK_FOOTPRINT_X / 2;
    const halfZ = BLOCK_FOOTPRINT_Z / 2;

    // Vertical street segment to the right
    const right = blockByGrid.get(
      `${block.district}:${block.signX}:${block.signZ}:${block.gx + 1},${block.gz}`,
    );
    if (right) {
    ...
    const bottom = blockByGrid.get(
      `${block.district}:${block.signX}:${block.signZ}:${block.gx},${block.gz + 1}`,
    );
    if (bottom) {
      // Skip roads crossing the river channel (which are handled by bridges)
      if (!(
        (block.gz < 0 && bottom.gz >= 0) ||
        (block.gz >= 0 && bottom.gz < 0)
      )) {
  ...
  for (const { type, count } of MONUMENT_TYPES) {
    for (let mi = 0; mi < count; mi++) {
      ...
      const angle =
        idx * goldenAngle + seededRandom(scatterSeed + idx * 7) * 0.4;
      const mx =
        Math.cos(angle) * r + (seededRandom(scatterSeed + idx * 13) - 0.5) * 20;
      let mz =
        Math.sin(angle) * r + (seededRandom(scatterSeed + idx * 17) - 0.5) * 20;
  ...
  for (let di = 0; di < districtZones.length; di++) {
    ...
    if (canalLength > 40) {
      const canalZ = cz > 0 ? cz - canalLength / 2 : cz + canalLength / 2;
      canals.push({
  ...
  for (let oi = 0; oi < parallelOffsets.length; oi++) {
    ...
    const hasBuildingsNear = buildings.some(
      (b) => Math.abs(b.position[2] - offZ) < 120,
    );
