...
describe("arena hidden-test encryption", () => {
  ...
  afterEach(() => {
    if (originalArenaCryptoKey === undefined) {
      ...
      process.env.ARENA_CRYPTO_KEY = originalArenaCryptoKey;
    }
  });
  ...
  it("encrypts hidden tests using a configured key", () => {
    ...
    expect(result.encryptedData).not.toContain(JSON.stringify(tests));
  });
  ...
  it("produces output that can be decrypted with the configured key", () => {
    ...
    expect(
      decryptHiddenTests(result.encryptedData, result.iv, validArenaCryptoKey),
    ).toEqual(tests);
  });
