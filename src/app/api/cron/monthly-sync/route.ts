...
export async function GET(request: Request) {
  ...
  try {
    ...
    const hfRes = await fetch(
      `https://huggingface.co/datasets/${DATASET_REPO}/resolve/main/data.jsonl`,
      {
    ...
    console.log(
      `Unseeded available -> Easy: ${unseededEasy.length}, Medium: ${unseededMedium.length}, Hard: ${unseededHard.length}`,
    );

    // We need 21 of each for the month
    const TARGET = 21;
    if (
      unseededEasy.length < TARGET ||
      unseededMedium.length < TARGET ||
      unseededHard.length < TARGET
    ) {
      return NextResponse.json(
        { error: "Not enough unseeded problems remaining in HF dataset." },
        { status: 400 },
      );
    }
    ...
    console.log(
      `Upserting ${formattedForSupabase.length} problems to Supabase...`,
    );
    const { error: insErr } = await sb
      .from("arena_problems")
      .upsert(formattedForSupabase, { onConflict: "source,source_id" });
    if (insErr) throw insErr;

    // 7. Update HF Dataset state in memory (mark seeded: true)
    console.log("Updating HF dataset state...");
    const updatedProblemsText = allProblems
      .map((p) => {
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully rotated ${formattedForSupabase.length} problems for the month.`,
    });
  } catch (err: unknown) {
    console.error(err);
