...
> = {
  ...
  pt: {
    ...
    wins: "vence",
    tie: "Empate",
    notFound: "Comparacao nao encontrada",
    cta: `Quem ganha? \u2192 ${APP_URL}`,
  },
};

const TRASH_TALK: Record<
  Lang,
  { stomp: string[]; win: string[]; close: string[]; tie: string[] }
> = {
  en: {
    stomp: ["THAT WASN'T EVEN CLOSE", "CALL AN AMBULANCE", "TOTAL DESTRUCTION"],
    win: ["BETTER LUCK NEXT TIME", "GET BACK TO CODING", "NOT EVEN A CONTEST"],
    close: ["THAT WAS PERSONAL", "DOWN TO THE WIRE", "PHOTO FINISH"],
    tie: ["PERFECTLY BALANCED", "COULDN'T PICK A WINNER", "REMATCH REQUIRED"],
  },
  pt: {
    stomp: ["NEM FOI JOGO", "CHAMA O SAMU", "DESTRUICAO TOTAL"],
    win: ["TENTA DE NOVO", "VOLTA PRO VSCODE", "SEM CHANCE"],
    close: ["ISSO FOI PESSOAL", "DECIDIDO NO DETALHE", "QUASE EMPATE"],
    tie: [
...
export async function GET(
  ...
  const lang = (
    request.nextUrl.searchParams.get("lang") === "pt" ? "pt" : "en"
  ) as Lang;
  const t = i18n[lang];
  ...
  const [{ data: devA }, { data: devB }] = await Promise.all([
    supabase
      .from("developers")
      .select(fields)
      .ilike("github_login", userA)
      .single(),
    supabase
      .from("developers")
      .select(fields)
      .ilike("github_login", userB)
      .single(),
  ]);
  ...
  const contribsA =
    ((devA as Record<string, unknown>).contributions_total as number) ||
    ((devA as Record<string, unknown>).contributions as number);
  const contribsB =
    ((devB as Record<string, unknown>).contributions_total as number) ||
    ((devB as Record<string, unknown>).contributions as number);
...
function renderLandscape(
  devA: Record<string, unknown>,
  devB: Record<string, unknown>,
  statRows: {
    label: string;
    a: number;
    b: number;
  ...
  const maxContrib = Math.max(
    devA.contributions as number,
    devB.contributions as number,
    1,
  );
  const MIN_H = 180;
  const MAX_H = 360;
  const heightA = Math.round(
    MIN_H + ((devA.contributions as number) / maxContrib) * (MAX_H - MIN_H),
  );
  const heightB = Math.round(
    MIN_H + ((devB.contributions as number) / maxContrib) * (MAX_H - MIN_H),
  );
  const GROUND_Y = 510;
  const BLDG_W = 180;
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
          ...
          >
            {((devA.name ?? devA.github_login) as string).slice(0, 14)}
          </div>
          <div style={{ display: "flex", fontSize: 14, color: muted }}>
            @{devA.github_login as string}
          </div>
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          >
            {((devB.name ?? devB.github_login) as string).slice(0, 14)}
          </div>
          <div style={{ display: "flex", fontSize: 14, color: muted }}>
            @{devB.github_login as string}
          </div>
        </div>
        {devB.avatar_url ? (
          <img
            src={devB.avatar_url as string}
            width={64}
            height={64}
    ...
    >
      ...
      >
        {statRows.map((s) => (
          <div
            key={s.label}
            style={{ display: "flex", alignItems: "center", width: 900 }}
          >
            <div
              style={{
                display: "flex",
