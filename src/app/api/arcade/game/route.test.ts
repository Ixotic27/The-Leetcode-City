...
describe("arcade game route", () => {
  ...
  beforeEach(() => {
    ...
    vi.resetAllMocks();
    // default auth user
    (mockSb.auth.getUser as ReturnType<typeof vi.fn>).mockResolvedValue({
  ...
  it("passes a null developer id through when no developer profile is linked", async () => {
    ...
    const signature = crypto
      ...
      .update(`user-1:10s_classic:${startTime}`)
      .digest("hex");
    const reqBody = {
      action: "stop",
      game: "10s_classic",
  ...
  it("processes concurrent submits with one winning RPC", async () => {
        ...
        return {
          select: () => ({
            eq: () => ({
              limit: () => ({
                data:
                  _table === "developers" ? [{ id: 123 }] : ([] as DevRow[]),
              }),
            }),
          }),
        } as unknown as ReturnType<typeof vi.fn>;
    ...
    const anyPx =
      (json1.result?.px_earned ?? 0) + (json2.result?.px_earned ?? 0);
