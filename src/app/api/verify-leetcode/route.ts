...
export async function POST(req: Request) {
  try {
    ...
    if (aboutMe === null) {
      return NextResponse.json(
        { error: "Could not find this LeetCode account" },
        { status: 404 },
      );
    }
    ...
    try {
      ...
      console.warn(
        "[app/api/verify-leetcode/route.ts] non-critical error:",
        err,
      );
    }
    ...
    const getAC = (d: string) =>
      acNums.find((x) => x.difficulty === d)?.count ?? 0;
    const getTotal = (d: string) =>
      totalNums.find((x) => x.difficulty === d)?.count ?? 1;
    ...
    const lc_badges_all = badges.map((b) => ({
      name: b.name,
      icon: b.icon,
    ...
    const lc_tag_stats = [
      ...
      .sort(
        (a: TagProblem, b: TagProblem) => b.problemsSolved - a.problemsSolved,
      )
      .slice(0, 20)
    ...
    const litPercentage = Math.min(
      0.92,
      Math.max(0.15, active_days_last_year / 365),
    );
    ...
    const name =
      lcUserStats?.profile?.realName ||
      lcUserStats?.username ||
      leetcode_username;
    ...
    const newXpTotal = mergeBaseXp(
      existingDev?.xp_total,
      existingDev?.xp_github,
      newBaseXp,
    );
    ...
    if (existingDev) {
      ...
      if (updateError || !updateData) {
        ...
        return NextResponse.json(
          {
            error:
              "This LeetCode account was just claimed by another user. Please try again.",
          },
          { status: 409 },
        );
      }
    ...
    return NextResponse.json({
      success: true,
      leetcode_username: leetcode_username.toLowerCase(),
    });
  } catch (err: unknown) {
    const message =
