...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.warn("[app/api/customizations/upload/route.ts] error:", err);
  ...
  const detectedType = detectMimeFromBytes(
    new Uint8Array(fileBuffer.slice(0, MAGIC_BYTES_TO_READ)),
  );
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    console.warn(
      "[app/api/customizations/upload/route.ts] non-critical storage cleanup error:",
      err,
    );
  }
