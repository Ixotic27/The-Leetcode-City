...
export async function POST(request: Request) {
  ...
  if (item.price_points === null || item.price_points === undefined) {
    return NextResponse.json(
      { error: "This item cannot be bought with points" },
      { status: 400 },
    );
  }
  ...
  if (!isConsumable) {
    ...
    if ((devFreeze?.streak_freezes_available ?? 0) >= 2) {
      return NextResponse.json(
        { error: "Max 2 streak freezes stored" },
        { status: 409 },
      );
    }
  }
  ...
  if (finalizeError) {
    ...
    console.error(
      "[buy-with-points] finalize_points_purchase failed:",
      finalizeError,
    );
    return NextResponse.json(
  ...
  if (!finalizeResult.ok) {
    ...
    if (finalizeResult.error_code === "not_enough_points") {
      return NextResponse.json(
        {
          error:
            "Not enough points or a concurrent purchase already deducted your balance. Please try again.",
        },
        { status: 409 },
      );
    }
  ...
  return NextResponse.json({
    ok: true,
    points_remaining: finalizeResult.points_remaining,
  });
}
