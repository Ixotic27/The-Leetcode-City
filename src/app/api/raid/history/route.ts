...
export async function GET(request: Request) {
  ...
  const [
    ...
    admin
      .from("raids")
      .select(
        "id, attacker_id, defender_id, success, created_at, attacker:developers!raids_attacker_id_fkey(github_login), defender:developers!raids_defender_id_fkey(github_login)",
      )
  ...
  const allRaids = [
    ...
    .map((r) => ({
      ...
      attacker_login:
        ...
        "unknown",
      defender_login:
        (r.defender as unknown as { github_login: string })?.github_login ??
        "unknown",
      success: r.success,
