...
export default async function Image({
  ...
  const winnerLogin =
    aWinsCount > bWinsCount ? devA.github_login : devB.github_login;
  const summary = isTie
  ...
  const heightA = Math.round(
    MIN_H + (contribsA / maxContrib) * (MAX_H - MIN_H),
  );
  const heightB = Math.round(
    MIN_H + (contribsB / maxContrib) * (MAX_H - MIN_H),
  );
  const GROUND_Y = 520;
  ...
  return new ImageResponse(
    ...
    >
      ...
      >
        ...
        <span style={{ fontSize: 16, color: muted, marginLeft: 8 }}>
          by ixotic
        </span>
      </div>
    </div>,
