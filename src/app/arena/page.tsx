...
export default function ArenaPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (session) {
        ...
        if (username) {
          const statsRes = await fetch(`/api/arena/stats/${username}`, {
            headers,
          });
          if (statsRes.ok) {
  ...
  const handleUseItem = async (itemId: string, itemName: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const actionStr =
          data.action === "consumed"
            ? "Consumed"
            : data.action === "equipped"
              ? "Equipped"
              : "Unequipped";
        setMessage({
          text: `Successfully ${actionStr.toLowerCase()} ${itemName}!`,
          type: "success",
        });
  ...
  const getDifficultyBorder = (diff: string) => {
    switch (diff) {
      ...
      case "medium":
        return "border-[#ffa116]";
      case "hard":
        return "border-[#ff0055]";
      default:
  ...
  const getDifficultyBadgeColor = (diff: string) => {
    switch (diff) {
      ...
      case "medium":
        return "bg-[#ffa116]/20 text-[#ffa116] border-[#ffa116]";
      case "hard":
        return "bg-[#ff0055]/20 text-[#ff0055] border-[#ff0055]";
      default:
    ...
    <main className="min-h-screen bg-bg font-pixel uppercase text-warm">
      <div className="mx-auto max-w-7xl px-4 py-8">
        ...
        <div className="flex flex-col items-center justify-between border-b border-border pb-6 md:flex-row">
          ...
          <h1
            className="text-center text-3xl tracking-widest text-cream"
            style={{ textShadow: `0 0 10px ${ACCENT_ARENA}44` }}
          >
        ...
        {message && (
          <div
            className={`mt-4 border-2 p-3 text-center text-xs normal-case ${
            ...
            <div className="flex flex-col gap-6 lg:col-span-8">
              <div className="border-[3px] border-border bg-bg-raised p-5">
                <h2
                  ...
                  style={{ color: ACCENT_ARENA }}
                >
                  Today's Daily Challenges
                </h2>
                <p className="mt-2 text-[10px] text-muted normal-case leading-relaxed">
                  Earn rating points, experience points (XP), and unlock
                  pixel-art RPG item drops by solving today's challenges. Click
                  a card to read the description and open the problem in your
                  IDE.
                </p>
                      ...
                      return (
                        <div
                          ...
                          className={`border-[3px] bg-bg-card p-4 transition-all duration-200 ${getDifficultyBorder(ch.difficulty)} ${
                            isExpanded
                              ? "shadow-[0_0_15px_rgba(255,161,22,0.15)]"
                              : ""
                          }`}
                        >
                          <div
                            ...
                            onClick={() =>
                              setExpandedChallenge(isExpanded ? null : ch.id)
                            }
                          >
                            <div className="flex flex-wrap items-center gap-3">
                              <span
                                className={`border px-2 py-0.5 text-[9px] font-bold ${getDifficultyBadgeColor(ch.difficulty)}`}
                              >
                                {ch.difficulty}
                              </span>
                              {ch.status === "accepted" && (
                            ...
                            <div className="flex items-center gap-4">
                              <div className="text-right">
                                <span className="text-[10px] text-cream block">
                          ...
                          {isExpanded && (
                            <div className="mt-4 border-t border-border pt-4 text-xs leading-relaxed text-muted normal-case">
                              <div className="mb-4 flex flex-wrap gap-5 border-b border-border/50 pb-3">
                                <div>
                                  <span className="text-cream text-[11px] uppercase font-bold block mb-0.5">
                                    CF Rating
                                  </span>
                                  <span className="text-[12px] text-cream font-bold block">
                                    {ch.problem.difficulty_rating}
                                  </span>
                                </div>
                                <div>
                                  <span className="text-cream text-[11px] uppercase font-bold block mb-0.5">
                              ...
                              <div className="mb-4">
                                <span className="text-cream text-[11px] uppercase font-bold block mb-1.5">
                                  Tags
                                </span>
                                <div className="flex flex-wrap gap-1.5">
                                  {ch.problem.tags.map((t) => (
                                    <span
                                      key={t}
                                      className="bg-bg-raised border border-border px-2.5 py-1 text-[10px] text-cream font-medium"
                                    >
                                      {t}
                                    </span>
                                  ))}
                              ...
                              <div className="mt-6 flex flex-col gap-2">
                                ...
                                <p className="text-center text-[10px] text-dim normal-case mt-1.5">
                                  Deep link requires the{" "}
                                  <span className="text-cream">
                                    LeetCode City: Pulse
                                  </span>{" "}
                                  extension installed in VS Code or Antigravity
                                  IDE.
                                </p>
                              </div>
                            </div>
              ...
              <div className="border-[3px] border-border bg-bg-raised p-5 text-xs leading-relaxed text-muted normal-case">
                <h3 className="text-xs text-cream uppercase mb-2">
                  ⚔ How to Compete
                </h3>
                <div className="space-y-3">
                  <div className="flex gap-2">
                    ...
                    <p>
                      ...
                      extension in VS Code or Antigravity IDE and log in using
                  ...
                  <div className="flex gap-2">
                    ...
                    <p>
                      ...
                      on any challenge to open the problem files side-by-side in
                  ...
                  <div className="flex gap-2">
                    ...
                    <p>
                      Write your solution. Click{" "}
                      <span className="text-cream font-bold">
                        "Run Samples"
                      </span>{" "}
                      in the editor sidebar to compile and execute your code
                      against sample test cases locally.
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <span
                    ...
                    <p>
                      Once sample tests pass, click{" "}
                      <span className="text-cream font-bold">"Submit"</span>.
                      The extension decrypts and runs the code against the full
                      hidden test suite, submits results to the server, and
                      awards items!
                    </p>
                  </div>
                </div>
              ...
              {session ? (
                  ...
                  {userStats ? (
                    <div className="mt-4 grid grid-cols-2 gap-4">
                      <div className="border border-border bg-bg-card p-3 text-center">
                        <span className="text-[8px] text-muted block mb-1">
                          Grid Score
                        </span>
                        <span
                          className="text-sm text-cream font-bold"
                          style={{ color: ACCENT_ARENA }}
                        >
                          {userStats.rating}
                        </span>
                      ...
                      <div className="border border-border bg-bg-card p-3 text-center">
                        ...
                        <span className="text-sm text-[#39ff14] font-bold">
                          🏆 {userStats.best_streak} days
                        </span>
                      </div>
                    </div>
                  ) : (
                    <div className="mt-3 text-[10px] text-muted normal-case">
                      No Arena stats yet. Solve your first daily challenge to
                      establish rating!
                    </div>
                  )}
                ...
                <div className="border-[3px] border-border bg-bg-raised p-5 text-center">
                  ...
                  <p className="text-[9px] text-muted normal-case mb-4">
                    Sign in with LeetCode to track Grid Score, streaks,
                    inventory, and buffs!
                  </p>
                  <Link
                    href="/shop"
                ...
                <div className="border-[3px] border-border bg-bg-raised p-5">
                  ...
                  {activeBuffs.length === 0 ? (
                    <p className="text-[9px] text-muted normal-case">
                      No active buffs. Drink potions or equip items to activate
                      XP boosts!
                    </p>
                  ) : (
                    <div className="space-y-3">
                      {activeBuffs.map((buff) => (
                        <div
                          key={buff.id}
                          className="border border-border bg-bg-card p-2.5 text-[10px] normal-case"
                        >
                    ...
                    <div className="flex flex-col gap-3 max-h-[350px] overflow-y-auto pr-1 scrollbar-thin">
                      {inventory.map((inv) => {
                        const isEquippable = [
                          ...
                          "legendary",
                          "companion",
                        ].includes(inv.item.item_type);
                        const isActionLoading = actionLoading === inv.item.id;

                        return (
                          <div
                            key={inv.id}
                            className="border border-border bg-bg-card p-2.5 flex items-start gap-3"
                          >
                            {/* Icon */}
                            <div
                              className="relative border-2 border-border bg-bg p-1 flex-shrink-0 flex items-center justify-center"
                            ...
                            <div className="flex-grow min-w-0 normal-case text-[10px]">
                              <div className="flex justify-between items-start">
                                <h3
                                  className="font-bold text-cream truncate max-w-[120px]"
                                  style={{
                                    color: getRarityColor(inv.item.rarity),
                                  }}
                                >
                                  {inv.item.name}
                                </h3>
                              ...
                              <button
                                ...
                                onClick={() =>
                                  handleUseItem(inv.item.id, inv.item.name)
                                }
              ...
              <div className="border-[3px] border-border bg-bg-raised p-5">
                ...
                <div className="space-y-2">
                  {leaderboard.length === 0 ? (
                    <p className="text-[9px] text-muted normal-case">
                      No coders ranked yet.
                    </p>
                  ) : (
                    leaderboard.slice(0, 5).map((entry, idx) => (
                      <div
                        key={`${entry.github_login || "entry"}-${idx}`}
                        className="flex justify-between items-center text-[10px] border-b border-border/50 pb-1.5"
                      >
                        <div className="flex items-center gap-2">
                          <span className="text-muted font-bold w-4">
                            #{idx + 1}
                          </span>
                        ...
                        <div className="flex items-center gap-3">
                          <span className="text-[#39ff14] text-[8px]">
