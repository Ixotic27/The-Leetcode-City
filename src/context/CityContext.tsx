...
interface CityContextProps {
  ...
  setRelicFocus: React.Dispatch<
    React.SetStateAction<{ x: number; y: number; z: number } | null>
  >;
  ...
  setWeatherMode: React.Dispatch<
    React.SetStateAction<"sunny" | "rainy" | "windy" | "stormy" | "snowy">
  >;
  hud: { speed: number; altitude: number };
  setHud: React.Dispatch<
    React.SetStateAction<{ speed: number; altitude: number }>
  >;
  ...
  setDistrictAnnouncement: React.Dispatch<
  ...
  setSelectedBuilding: React.Dispatch<
    React.SetStateAction<CityBuilding | null>
  >;
  shareData: {
    login: string;
  ...
  setComparePair: React.Dispatch<
    React.SetStateAction<[CityBuilding, CityBuilding] | null>
  >;
  ...
  setGiftedInfo: React.Dispatch<
    React.SetStateAction<{ item: string; to: string } | null>
  >;
  ...
  failedUsernamesRef: React.MutableRefObject<
...
export function CityProvider({ children }: { children: ReactNode }) {
  ...
  const failedUsernamesRef = useRef<
    Map<string, { code: string; timestamp: number }>
  >(new Map());
  ...
  const [relicFocus, setRelicFocus] = useState<{
    x: number;
    y: number;
    z: number;
  } | null>(null);
  ...
  const [hasVsCodeKey, setHasVsCodeKey] = useState<boolean | null>(() => {
    try {
      return localStorage.getItem("leetcodecity_has_vscode_key") === "1"
        ? true
        : null;
    } catch {
  ...
  const cycleWeather = () => {
    const modes: ("sunny" | "rainy" | "windy" | "stormy" | "snowy")[] = [
      "sunny",
      "rainy",
      "windy",
      "stormy",
  ...
  const isOwnBuilding = useMemo(() => {
    ...
    const matchesLinked =
      ...
      buildingLogin === linkedLeetCodeUsername.toLowerCase();
  ...
  const shopHref = useMemo(() => {
    return session && myBuilding?.claimed
  ...
  const mpAvatarUrl =
    myBuilding?.avatar_url ?? session?.user?.user_metadata?.avatar_url ?? null;
  const {
  ...
  const effectiveLiveCount =
    mpStatus === "connected" && mpPlayerCount > 0 ? mpPlayerCount : liveUsers;
  const effectiveLiveStatus =
    mpStatus === "connected" ? "connected" : liveStatus;
  ...
  const reloadCity = useCallback(async (bustCache = false) => {
    ...
    try {
      ...
      console.warn(
        "[city] Snapshot fetch failed; falling back to chunked city data.",
        err,
      );
    }
  ...
  const endFly = useCallback(
    (aborted = false) => {
      ...
      try {
        ...
        console.warn(
          "[fly] Failed to read personal best from localStorage:",
          err,
        );
      }
  ...
  const handleSignIn = useCallback(async () => {
    ...
    try {
      ...
      console.warn(
        "[auth] Failed to read referral metadata before GitHub login.",
        err,
      );
    }
  ...
  useEffect(() => {
    ...
    const updateSession = async (s: Session | null) => {
      ...
      if (s) {
        ...
        try {
            ...
            try {
              if (custs.custom_color?.color) {
                localStorage.setItem(
                  ...
                  JSON.stringify({
                    developerId: devId,
                    value: custs.custom_color.color,
                    ts: Date.now(),
                  }),
                );
              }
              ...
              if (custs.building_style?.style) {
                localStorage.setItem(
                  ...
                  JSON.stringify({
                    developerId: devId,
                    value: custs.building_style.style,
                    ts: Date.now(),
                  }),
                );
              }
              ...
              if (custs.selected_title?.slug) {
                localStorage.setItem(
                  ...
                  JSON.stringify({
                    developerId: devId,
                    value: custs.selected_title.slug,
                    ts: Date.now(),
                  }),
                );
              }
              ...
              console.warn(
                "Failed to set local storage overrides in session update:",
                err,
              );
            }
              ...
              prev.map((b) => {
                if (b.login.toLowerCase() === usernameLC) {
                  return {
                    ...
                    building_style:
                      custs.building_style?.style ?? b.building_style,
                    led_banner_text:
                      custs.led_banner?.text ?? b.led_banner_text,
                    selected_title:
                      custs.selected_title?.slug ?? b.selected_title,
                  };
                }
                return b;
              }),
          ...
          console.warn(
            "[profile] Failed to resolve linked LeetCode status.",
            err,
          );
          setLinkedLeetCodeUsername(null);
        } finally {
    ...
    supabase.auth
      .getSession()
      .then((res: { data: { session: Session | null }; error: any }) => {
        ...
        if (event !== "TOKEN_REFRESHED") {
          ...
          if (event === "SIGNED_IN" && s) {
            try {
              ...
              console.warn(
                "[auth] Failed to check leetcode_username after sign-in:",
                err,
              );
            }
          }
      ...
      try {
        ...
        if (foundIdx !== -1) {
          ...
          rawDevsRef.current[foundIdx] = {
            ...
            ...devData,
            loadout:
              devData.loadout !== undefined
                ? devData.loadout
                : (existing.loadout ?? null),
            custom_color:
              devData.custom_color !== undefined
                ? devData.custom_color
                : (existing.custom_color ?? null),
            owned_items:
              devData.owned_items !== undefined
                ? devData.owned_items
                : (existing.owned_items ?? []),
            billboard_images:
              devData.billboard_images !== undefined
                ? devData.billboard_images
                : (existing.billboard_images ?? []),
            building_style:
              devData.building_style !== undefined
                ? devData.building_style
                : (existing.building_style ?? "tower"),
          };
          const layout = generateCityLayout(rawDevsRef.current);
          setBuildings(layout.buildings);
          setPlazas(layout.plazas);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (
        ["sunny", "rainy", "windy", "stormy", "snowy"].includes(
          savedWeather ?? "",
        )
      ) {
  ...
  useEffect(() => {
    if (codingPanelOpen && hasVsCodeKey === null) {
      ...
      fetch(`/api/vscode-key?t=${Date.now()}`, {
        cache: "no-store",
    ...
    if (
      ...
      const timer = setTimeout(async () => {
        try {
          ...
          console.warn(
            "[missions] Failed to track building visit mission.",
            err,
          );
        }
      }, 3000);
    ...
    async function loadCity() {
      try {
        ...
        try {
          ...
          console.warn(
            "[city] Snapshot fetch failed during load; falling back to chunked city data.",
            err,
          );
        }
  ...
  const handleBusArrival = useCallback(
  ...
  const handleOpenTransitMenu = useCallback(
    (fromDistrict: string) => {
  ...
  const handleSelectTransitDestination = useCallback(
    (toDistrict: string) => {
