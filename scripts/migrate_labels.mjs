...
async function main() {
  ...
  const labelsToCreate = [
    ...
    },
    {
      name: "level:critical",
      color: "6b7280",
      description: "Critical difficulty level",
    },
    {
      name: "Gssoc 26",
      color: "e5533d",
      description: "GSSoC 2026 contribution",
    },
    {
      name: "gssoc:approved",
      color: "0e8a16",
      description: "Approved GSSoC contribution",
    },
    {
  ...
  for (const label of labelsToCreate) {
    const res = await ghFetch(
      `${API}/labels/${encodeURIComponent(label.name)}`,
    );
    if (res.status === 404) {
      console.log(`  ➕ Creating: ${label.name}`);
      await ghFetch(`${API}/labels`, {
        method: "POST",
  ...
  console.log(
    `  Found ${allOpen.length} open + ${allClosed.length} closed = ${allItems.length} total`,
  );
  ...
  for (const item of allItems) {
    ...
    for (const [bare, leveled] of Object.entries(MIGRATE_MAP)) {
      if (currentLabels.includes(bare)) {
        ...
        if (
          !currentLabels.includes(leveled) &&
          !labelsToAdd.includes(leveled)
        ) {
    ...
    const allCurrentAndAdded = [...currentLabels, ...labelsToAdd].filter(
      (l) => !labelsToRemove.includes(l),
    );
    const hasDifficulty = allCurrentAndAdded.some((l) =>
      DIFFICULTY_LABELS.includes(l),
    );
    if (!hasDifficulty) {
    ...
    console.log(
      `${type} #${num}: +[${labelsToAdd.join(", ")}] -[${labelsToRemove.join(", ")}]`,
    );
    ...
    for (const label of labelsToRemove) {
      await ghFetch(
        `${API}/issues/${num}/labels/${encodeURIComponent(label)}`,
        {
          method: "DELETE",
        },
      );
  ...
  for (const label of LABELS_TO_REMOVE) {
    const res = await ghFetch(`${API}/labels/${encodeURIComponent(label)}`, {
      method: "DELETE",
    });
