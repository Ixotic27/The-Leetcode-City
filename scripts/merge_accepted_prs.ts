...
async function mergePR(prNumber: number) {
  ...
  if (reviewRes.ok) {
    ...
    console.warn(
      `  ⚠️ Failed to approve PR #${prNumber}:`,
  ...
  if (prRes.ok) {
    ...
    const existingLabels: string[] =
      prData.labels?.map((l: any) => l.name) || [];
    ...
    console.warn(
      `  ⚠️ Failed to fetch PR data for label verification:`,
      await prRes.text(),
    );
  }
...
async function run() {
  console.log(
    `Starting to merge ${prsToMerge.length} accepted pull requests...\n`,
  );
