    ...
    const req = https.request(options, (res) => {
      ...
      res.on("end", () => {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          ...
          reject(
            new Error(
              `GraphQL failed with status ${res.statusCode}: ${resBody}`,
            ),
          );
        }
      });
...
async function run() {
  ...
  try {
    ...
    const searchResults = await restRequest(
      "GET",
      `/search/issues?q=repo:${repo}+state:open+dark+continent`,
    );
    const items = searchResults.items || [];
    if (items.length > 0) {
      for (const item of items) {
        console.log(
          `Closing and locking issue #${item.number}: "${item.title}"`,
        );
        // Close the issue
        ...
        await restRequest(
          "POST",
          `/repos/${repo}/issues/${item.number}/comments`,
          {
...
Please put your thoughts, ideas, and architecture designs in the comments below. Let's build LeetCode City together!`;
    ...
    console.log(
      `Created new issue #${newIssueNumber} (Node ID: ${newIssueNodeId})`,
    );
  } catch (e) {
  ...
  if (newIssueNodeId) {
    try {
      ...
      const pinMutation = `
            ...
            `;
      const pinResult = await graphqlRequest(pinMutation, {
      ...
      if (pinResult.errors) {
        ...
        console.log(
          "Successfully pinned issue:",
          pinResult.data.pinIssue.issue.title,
        );
      }
    } catch (e) {
  ...
  try {
    ...
    const discTitle =
      "Introducing Supabase Realtime Multiplayer & Open Map Development";
...
Thank you for your incredible support in making LeetCode City amazing!`;
    ...
    if (discResult.errors) {
      ...
      console.log(
        ...
        discResult.data.createDiscussion.discussion.url,
      );
    }
  } catch (e) {
