...
async function run() {
  try {
    ...
    const targetTitles = {
      ...
      dailyBuilding:
        "Redesign Daily Questions Building to Match E.Arcade Style",
      traffic:
        "Replace Tower-Orbiting Traffic with Optimized Street/Ring Random Cars",
    };

    const mapThemeLabels = [
      "gssoc 26",
      "gssoc:approved",
      "level:advanced",
      "type:feature",
    ...
    for (const issue of issues) {
      ...
      if (labelsToSet) {
        console.log(
          `Updating labels for issue #${issue.number}: "${issue.title}"`,
        );
