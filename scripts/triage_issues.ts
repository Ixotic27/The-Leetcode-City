...
async function triageIssue(issue: (typeof issuesToTriage)[0]) {
  ...
  if (labelRes.status === 200) {
    ...
    console.error(
      `❌ Failed to update labels for #${issue.number}:`,
      await labelRes.text(),
    );
    return;
  }
  ...
  const assignRes = await fetch(
    `https://api.github.com/repos/${repo}/issues/${issue.number}/assignees`,
    {
      method: "POST",
      headers,
  ...
  if (assignRes.status === 201) {
    ...
    console.error(
      `❌ Failed to assign @${issue.author} to #${issue.number}:`,
...
🔒 *This conversation will now be locked. Please use your Pull Request for any further discussion about this issue.*`;
  ...
  if (commentRes.status === 201) {
    ...
    console.error(
      `❌ Failed to post comment on #${issue.number}:`,
  ...
  const lockRes = await fetch(
    `https://api.github.com/repos/${repo}/issues/${issue.number}/lock`,
    {
      method: "PUT",
      headers,
  ...
  if (lockRes.status === 204) {
    ...
    console.error(
      `❌ Failed to lock issue #${issue.number}:`,
      await lockRes.text(),
    );
  }
}
