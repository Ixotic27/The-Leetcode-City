...
async function main() {
  ...
  let res = await fetch(
    `https://api.github.com/repos/${repo}/labels/gssoc:approved`,
    { headers },
  );
  ...
  if (!approvedExists) {
    console.log(
      "gssoc:approved does not exist. Renaming gssoc:verified to gssoc:approved...",
    );
    const patchRes = await fetch(
      `https://api.github.com/repos/${repo}/labels/gssoc:verified`,
      {
        method: "PATCH",
        headers,
        body: JSON.stringify({
          new_name: "gssoc:approved",
          color: "0e8a16",
  ...
  for (let page = 1; page <= 50; page++) {
    const issuesRes = await fetch(
      `https://api.github.com/repos/${repo}/issues?state=all&labels=gssoc:verified&per_page=100&page=${page}`,
      { headers },
    );
  ...
  await fetch(`https://api.github.com/repos/${repo}/labels/gssoc:verified`, {
    method: "DELETE",
    headers,
  });
  console.log("Done!");
