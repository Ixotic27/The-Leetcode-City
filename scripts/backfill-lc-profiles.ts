...
async function upsertFullProfile(
  ...
  const dominantLanguage =
    languages.length > 0
      ? [...languages].sort(
          (a: any, b: any) => b.problemsSolved - a.problemsSolved,
        )[0].languageName
      : null;
...
async function main() {
  ...
  console.log(
    `   Found ${logins.length} developers. Starting backfill at ~${Math.round(3600 / (DELAY_MS / 1000))} users/hour...\n`,
  );
  console.log(
    "   Press Ctrl+C at any time — all progress is saved via upsert.\n",
  );
  ...
  for (let i = 0; i < logins.length; i++) {
    ...
    process.stdout.write(
      `  [${String(i + 1).padStart(5)}/${logins.length}] (${pct}%) ${username.padEnd(25)} `,
    );
    ...
    if (!data?.matchedUser) {
      ...
      if (success) {
        ...
        console.log(
          `✅  ${solved} solved | streak ${streak} | ${badges} badge(s)`,
        );
        ok++;
      } else {
  ...
  console.log(
    `   Success: ${ok} | Skipped (private/not found): ${skip} | Failed: ${fail}`,
  );
  console.log(
    `   ETA was: ~${Math.round(((logins.length * DELAY_MS) / 3600000) * 10) / 10} hours\n`,
  );
}
