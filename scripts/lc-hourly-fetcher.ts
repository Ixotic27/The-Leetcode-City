  ...
  for (const year of yearsToCheck) {
    ...
    if (calendarStr) {
      try {
        ...
        console.warn(
          `  [lc-refresh] Error parsing calendar for year ${year} of ${username}:`,
          err,
        );
      }
    }
...
async function filterNewUsernames(usernames: string[]): Promise<string[]> {
  ...
  const existingSet = new Set(
    (existing ?? []).map((d: { github_login: string }) => d.github_login),
  );
...
async function discoverNewUsers(pagesToScan: number): Promise<number> {
  ...
  for (let i = 0; i < pagesToScan; i++) {
    ...
    if (usernames.length === 0) {
      console.log(
        `  ⚠️  No users found on ranking page ${page}. Rate limited?`,
      );
      await sleep(5000);
      continue;
    }
    ...
    if (newUsernames.length === 0) {
      console.log(
        `  Page ${page}: all ${usernames.length} users already in DB`,
      );
      continue;
    }

    console.log(
      `  Page ${page}: ${newUsernames.length} new users out of ${usernames.length}`,
    );
    ...
    if (error) {
      ...
      console.log(
        `  ✅ Inserted ${newUsernames.length} stubs from page ${page}`,
      );
    }
...
async function pickStalestDevs(n: number): Promise<string[]> {
  ...
  const claimedLogins = (claimed ?? []).map(
    (d: { github_login: string }) => d.github_login,
  );
  ...
  return [
    ...claimedLogins,
    ...(unclaimed ?? []).map((d: { github_login: string }) => d.github_login),
  ];
}
...
async function runHourlyCycle(cycleNum: number) {
  ...
  console.log(
    `  Refreshing ${logins.length} profiles (target: ${LIMIT}, concurrency: ${CONCURRENCY})...\n`,
  );
  ...
  async function fetchAndUpsert(
    ...
    if (!data?.matchedUser) {
      console.log(
        `  [${String(index + 1).padStart(3)}] ${username.padEnd(28)} ⚠️  not found / private`,
      );
      skip++;
    } else {
      ...
      if (success) {
        ...
        console.log(
          `  [${String(index + 1).padStart(3)}] ${username.padEnd(28)} ✅  ${solved} solved | streak ${streak}`,
        );
        ok++;
      } else {
        console.log(
          `  [${String(index + 1).padStart(3)}] ${username.padEnd(28)} ❌ DB error`,
        );
        fail++;
      }
    }
  ...
  for (let i = 0; i < logins.length; i += CONCURRENCY) {
    ...
    await Promise.allSettled(
      chunk.map((username, index) => fetchAndUpsert(username, i + index)),
    );
