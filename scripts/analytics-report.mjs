...
async function main() {
  ...
  // Top devs by visits
  const topVisits = [...devs]
    .sort((a, b) => b.visit_count - a.visit_count)
    .slice(0, 10);

  // Top devs by contributions
  const topContribs = [...devs]
  ...
  const avgStreak =
    activeStreaks.length > 0
      ? (
          activeStreaks.reduce((s, d) => s + d.app_streak, 0) /
  ...
  const oldestAccounts = devs
    ...
    .sort(
      (a, b) => new Date(a.account_created_at) - new Date(b.account_created_at),
    )
    .slice(0, 10);
  const newestAccounts = devs
    .filter((d) => d.account_created_at)
    .sort(
      (a, b) => new Date(b.account_created_at) - new Date(a.account_created_at),
    )
    .slice(0, 5);
...
|---------|--------|
`;
  for (const [plan, count] of Object.entries(adsByPlan).sort(
    ([, a], [, b]) => b - a,
  )) {
    report += `| ${plan} | ${count} |\n`;
  }
...
|---------|--------|
`;
  for (const [v, count] of Object.entries(adsByVehicle).sort(
    ([, a], [, b]) => b - a,
  )) {
    report += `| ${v} | ${count} |\n`;
  }
