...
for (const file of files) {
  ...
  const oldLoginRegex =
    /const githubLogin = \(\s*user\.user_metadata\?\.user_name \?\?\s*user\.user_metadata\?\.preferred_username \?\?\s*""\s*\)\.toLowerCase\(\);/gs;
  if (oldLoginRegex.test(content)) {
    content = content.replace(oldLoginRegex, "");
    changed = true;
  }

  const oldLoginRegex2 =
    /const githubLogin = \(\s*user\.user_metadata\.user_name \?\?\s*user\.user_metadata\.preferred_username \?\?\s*""\s*\)\.toLowerCase\(\);/gs;
  ...
  const oldLoginRegex3 =
    /const login = user\.user_metadata\?\.user_name\?\.toLowerCase\(\);/g;
