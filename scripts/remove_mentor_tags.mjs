...
async function removeMentorTags() {
  ...
  const res = await fetch(
    `https://api.github.com/repos/${repo}/issues?labels=mentor:Ixotic27&state=all&per_page=100`,
    { headers },
  );
  ...
  for (const issue of issues) {
    ...
    const deleteRes = await fetch(
      `https://api.github.com/repos/${repo}/issues/${issue.number}/labels/mentor:Ixotic27`,
      {
        method: "DELETE",
        headers,
      },
    ...
    if (!deleteRes.ok) {
      console.error(
        `Failed to remove tag from issue #${issue.number}:`,
